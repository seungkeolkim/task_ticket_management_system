...
루트 `/`는 내 작업 대시보드이며 로그인하지 않았다면 로그인 화면으로 이동합니다. 인증, 사용자·조직 조회·생성, 프로젝트 생성·조회·참여자 등록, 티켓 생성·조회·편집·상태 전이, 내 작업 대시보드와 칸반 상태 이동은 실제 DB에 연결되어 있습니다. 티켓 관계·휴지통·댓글·멘션 쓰기·첨부파일과 저장 필터는 후속 범위입니다. 최초 실행 전 [초기 관리자 설정](docs/authentication.md)에 따라 CLI 또는 bootstrap 환경 변수로 관리자를 생성하세요.
