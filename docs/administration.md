...
사용자·조직 관리 기능은 기존 identity 테이블을 사용한다. 현재 Alembic head는 Tiptap 본문 v2를 추가한 `20260926_0004`이며, 사용자·조직 테이블에는 별도 변경이 없다. 하위 조직이 있는 DB를 완전히 제거하는 `downgrade base`의 기존 FK 오류만 초기 revision의 downgrade에서 보완했다(DB-015). 이는 전체 데이터를 제거하는 운영 명령이며 화면 기능 사용을 위해 실행할 필요가 없다.

사용자 수정·비활성화·비밀번호 초기화, 조직 수정·이동·비활성화·JSON 입출력은 아직 연결하지 않았다. 프로젝트 생성·조회·참여자 추가는 [프로젝트 관리](projects.md)에 연결되어 있다. 티켓 생성·조회·편집·상태 전이·관계 관리·계층 단위 휴지통 이동과 복구, 대시보드와 칸반 상태 이동은 실제 DB에 연결되어 있다. 휴지통 영구 삭제, 댓글·멘션 쓰기와 첨부파일은 후속이다.
