...
사용자 수정·비활성화·비밀번호 초기화, 조직 수정·이동·비활성화·JSON 입출력은 아직 연결하지 않았다. 프로젝트 생성·조회·참여자 추가는 [프로젝트 관리](projects.md)에 연결되어 있다. 티켓 생성·조회·편집·상태 전이, 대시보드와 칸반 상태 이동은 실제 DB에 연결되어 있으며 관계·휴지통·댓글·멘션 쓰기·첨부파일은 후속이다.
