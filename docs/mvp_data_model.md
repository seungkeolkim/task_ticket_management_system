...
- 초기 번호 1부터 `projects.next_ticket_number`를 원자적으로 증가시키고 티켓 생성과 함께 commit한다. `MAX(number)`에서 계산하지 않는다. 발급 API와 동시성 검증은 후속이다.
...
- 기존 목업의 소문자 상태·표시명은 DB 코드가 아니다. 실제 UI 연결 시 변환 계층을 둔다.
...
- lag_days는 signed 달력 일수다. MVP 관계 생성 서비스는 Depends on에 FS/0을 넣고, Related는 dependency_kind=NULL/lag_days=0으로 저장한다.
...
로그인·비밀번호 변경 → 사용자·조직 기본 관리 → 프로젝트·참여자 → 티켓 생성·목록·상세 → 대시보드·칸반 조회 → 편집·상태 전이 → 협업·휴지통 확장.
