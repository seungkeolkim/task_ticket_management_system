...
초기 관리자나 비밀번호 초기화 대상은 먼저 `/account/password`에서 현재 비밀번호와 새 비밀번호를 입력해야 한다. 변경 시 모든 기기의 기존 세션을 제거하고 새 비밀번호로 다시 로그인한다. 인증과 사용자·조직의 조회·생성은 실제 DB에 연결되어 있다. 프로젝트·티켓 화면은 로그인 후 접근하는 예시 데이터이며 프로젝트 권한과 업무 쓰기는 후속 작업이다. `/admin/*`는 시스템 관리자만 접근한다.
