...
초기 관리자나 비밀번호 초기화 대상은 먼저 `/account/password`에서 현재 비밀번호와 새 비밀번호를 입력해야 한다. 변경 시 모든 기기의 기존 세션을 제거하고 새 비밀번호로 다시 로그인한다. 인증과 사용자·조직의 조회·생성, 프로젝트 권한, 티켓 생성·조회·편집·상태 전이·관계 관리·계층 단위 휴지통 이동과 복구, 대시보드와 칸반 이동은 실제 DB에 연결되어 있다. `/admin/*`는 시스템 관리자만 접근한다. 휴지통 영구 삭제, 댓글·멘션 쓰기, 첨부파일과 저장 필터는 아직 연결하지 않았다.
