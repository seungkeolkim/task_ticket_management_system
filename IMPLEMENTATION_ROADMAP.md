...
| 6 | 티켓 편집·FSM·이력·충돌 처리·칸반 이동 | 진행 중 — 칸반 이동 제외 완료 |
...
2026-09-23 티켓 편집·상태 전이 연결: 제목·설명 원문·중요도·담당자·마감일·상위 티켓 편집과 전용 FSM 상태 전이 API·HTML을 실제 데이터에 연결했다. creator·assignee·프로젝트 관리자와 감사되는 시스템 관리자 override, 비활성·종료 티켓 쓰기 차단, 계층 이동·순환 방어, 의존 대상 완료 차단, Epic 미완료 Task 확인, 최초 착수·완료·취소 시각, 필수 expected_version과 stale 409를 적용했다. 한 버전당 전체 전후 snapshot 이력 한 건과 감사 로그를 같은 transaction에 기록하고 no-op·실패·충돌 rollback을 검증한다. Windows Python 3.13에서 pytest 221개와 Ruff 검사를 통과했다. 관계 생성·삭제와 칸반 드래그는 후속이며 기존 head `20260917_0002`를 유지한다.
...
- [x] creator·assignee·프로젝트 관리자 수정 권한 구현
